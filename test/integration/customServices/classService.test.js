...
describe('INTEGRATION \'Class\' service', async () => {
  const name = 'kittens';
  ...
  it('missing methods are still missing', async () => {
    const service = appFactory(name, ServiceClass, {}, ['find']);
    assert.notEqual(typeof service.update, 'function');
    assert.notEqual(typeof service._update, 'function');
  });
