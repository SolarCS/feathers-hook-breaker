...
exports.standardFunctionTests = function (ServiceClass, name) {
  ...
  it('allowed method fails twice and returns CircuitBreakerError', async () => {
    ...
    global.breakers = {};
    const error1 = await service.get(99, p).catch(e => e.className);
    const error2 = await service.get(99, p).catch(e => e.className);
    const error3 = await service._get(99, p).catch(e => e.className);
  ...
  it('disallowed method fails twice and returns error', async () => {
    ...
    const error2 = await service.get(99, p).catch(e => e.className);
    const error3 = await service._get(99, p).catch(e => e.className);
    const error4 = await service._get(99, p).catch(e => e.className);
    assert.equal(error1, 'not-found');
  ...
  it('allowed method succeeds and does trigger events', async () => {
    ...
    global.breakers = {};
    const params = { breakerOptions: { onFailure, onSuccess } };
  ...
  it('allowed method fails and does trigger events', async () => {
    ...
    global.breakers = {};
    const params = { breakerOptions: { onFailure, onSuccess } };
    const error = await service.get(99, params).catch(e => e.className);
  ...
  it('skips hooks when calling _func without allowed _func', async () => {
    ...
    global.breakers = {};
    const result = await service._find(p).catch(e => e.className);
...
exports.customFunctionTests = function (ServiceClass, name) {
  ...
  it('allowed method fails twice and returns CircuitBreakerError', async () => {
    const service = appFactory(name, ServiceClass, {}, ['custom']);
    global.breakers = {};
    const error1 = await service.custom(99, p).catch(e => e.className);
    const error2 = await service.custom(99, p).catch(e => e.className);
    assert.equal(error1, 'not-found');
    assert.equal(error2, 'circuit-breaker-error');
  });
  ...
  it('disallowed method fails twice and returns error', async () => {
    const service = appFactory(name, ServiceClass, {}, ['custom']);
    global.breakers = {};
    const error1 = await service._custom(99, p).catch(e => e.className);
    const error2 = await service._custom(99, p).catch(e => e.className);
    assert.equal(error1, 'not-found');
    assert.equal(error2, 'not-found');
  });
  ...
  it('allowed method fails and does trigger events', async () => {
    ...
    global.breakers = {};
    const params = { breakerOptions: { onFailure, onSuccess } };
    const error = await service.custom(99, params).catch(e => e.className);
    assert.equal(error, 'not-found');
    assert.ok(onFailure.called);
    assert.ok(!onSuccess.called);
  });
