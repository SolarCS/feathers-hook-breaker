...
module.exports = (options = {}) => async ctx => {
  ...
  breaker.fallback((data) => {
    if (options.fallback) {
      return options.fallback(data, openBeforeCall);
    } else {
      return {
        status: 'fallback called',
        breakerState: openBeforeCall ? 'open' : 'closed',
        ...data
      };
    }
  });
  ...
  ctx.result = await breaker.fire({ id: ctx.id, data: ctx.data, params: ctx.params })
    .catch(error => {
      const output = options.verbose ? error : error.code + ' - ' + error.message;

      console.log('Non-\'failure\' Error Caught by Circuit Breaker: ', output);
