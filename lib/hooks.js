...
const beforeBreakerHook = (options) => async (ctx) => {
  const configOptions = ctx.app.get('breaker') || {};
  ctx.params = {
    ...ctx.params,
    breakerOptions: {
  ...
  if(typeof ctx.params.breakerOptions.name === 'function')
...
const errorBreakerHook = (fallback) => async (ctx) => {
  ...
  const name = ctx.params.breakerOptions.resolvedName;
  const breaker = global.breakers[name];
